...
/**
 * A JMX managed subclass of directory poller, exposing the {@link ManagedDirectoryPollerMBean
 * ManagedDirectoryPollerMBean} interface.
 * <p/>
 * From v1.5, JMX notifications are also emitted together with the existing {@link
 * org.sadun.util.polling.BasePollerEvent events}.
 * <p/>
...
public class ManagedDirectoryPoller
	...
	/**
	 * The thread in which the {@link DirectoryPoller DirectoryPoller} is actually run.
	 */
	...
	/**
	 * If JMX timing is used, there's no real thread running - the cycles are triggered by receiving timer events.
	 * <p/>
	...
	/**
	 * Implement the NotificationBroadcaster interface, by registering the listener at the internal {@link
	...
	/**
	 * Implement the NotificationBroadcaster interface, by removing the listener from the internal {@link
	 * JMXNotificationsPollManager}.
	 */
	...
	/**
	 * Remove a listening MBean (found in the only existing MBean server) via its ObjectName. Only one MBean server must be
	 * present.
	 *
	 * @param listenerMBeanName the object name of the mbean which is not to listen anymore to the poller's JMX
	 *                          notifications
	 */
